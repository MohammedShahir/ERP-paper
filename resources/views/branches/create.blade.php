@extends('layouts.app')
@section('title', __('messages.branch.add'))
@section('content')
    <h1 class="text-xl font-semibold mb-4">{{ __('messages.branch.add') }}</h1>
    <form action="{{ route('branches.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">@csrf
        ...
        <div class="md:col-span-2"><label class="block text-sm mb-1">{{ __('messages.branch.address') }}</label><input
                name="address" class="w-full rounded border p-2"></div>
        <div class="md:col-span-2 flex items-center gap-2"><a href="{{ route('branches.index') }}"
                ...
                class="px-3 py-2 rounded bg-[#1b1b18] text-white">{{ __('messages.actions.save') }}</button></div>
    </form>
@endsection
