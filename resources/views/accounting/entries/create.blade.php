...
@section('content')
    <h1 class="text-xl font-semibold mb-4">New Journal Entry</h1>
    <form action="{{ route('accounting.entries.store') }}" method="POST" x-data="entryForm()" class="space-y-4">@csrf
        ...
        <div class="rounded border bg-white overflow-hidden max-w-4xl">
            ...
            <div class="p-3"><button type="button" class="px-3 py-2 rounded border" @click="add()">Add Line</button>
            </div>
        </div>
        <div class="max-w-4xl flex items-center justify-end gap-6 text-sm">
            <div>Total Debit: $<span x-text="totalDebit().toFixed(2)"></span></div>
            <div>Total Credit: $<span x-text="totalCredit().toFixed(2)"></span></div>
        </div>
        <div class="max-w-4xl flex items-center gap-2">
            <a href="{{ route('accounting.entries.index') }}" class="px-3 py-2 rounded border">Cancel</a>
            <button class="px-3 py-2 rounded bg-[#1b1b18] text-white">Post Entry</button>
        </div>
    </form>
    <script>
        function entryForm() {
